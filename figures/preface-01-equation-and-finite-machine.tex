...
\begin{tikzpicture}[diagram]
  ...
  \node[diagram panel title] at (5.55,7.96) {기계의 다섯 조건과 실행 계획};
  ...
  \node[concept box,minimum width=28mm,minimum height=13mm] (time) at (10.92,6.35)
    {신호·연산 시간\\0이 아님};
  \node[concept box,minimum width=28mm,minimum height=13mm] (path) at (7.12,4.42)
    {이동 거리·통로 폭\\한 번에 얼마나?};
  \node[concept box,minimum width=28mm,minimum height=13mm] (store) at (10.92,4.42)
    {가까운 저장공간\\얼마나 둘까?};
  \node[concept box,minimum width=31mm,minimum height=13mm] (bits) at (9.02,2.70)
    {값의 비트 수\\범위와 정밀도};
  \node[concept emphasis,minimum width=63mm,minimum height=10mm] (plan) at (9.02,1.13)
    {다섯 조건을 엮은 실행 계획 · 배치·순서·재사용};

  \draw[concept arrow] (bits.south) -- (plan.north);
\end{tikzpicture}
\end{bookfigure}

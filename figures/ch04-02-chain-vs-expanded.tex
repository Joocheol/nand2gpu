...
\begin{tikzpicture}[diagram]
  ...
  \draw[path wire] (e-t2.east) -- ([yshift=1.7mm]e-u1.west);
  \draw[wire] (e-t3.east) -- ++(0.12,0) |- ([yshift=-1.7mm]e-u1.west);
  \draw[wire] (e-t4c.east) -- (e-t4.west);
  \draw[wire] (e-u0.east) -- ++(2.25,0) |- ([yshift=1.7mm]e-u2.west);
  \draw[path wire] (e-u1.east) -- ([yshift=-1.7mm]e-u2.west);
  \draw[path wire] (e-u2.east) -- ([yshift=1.7mm]e-c4.west);
  \draw[wire] (e-t4.east) -- ++(2.30,0) |- ([yshift=-1.7mm]e-c4.west);

  \node[diagram footer] at (6.50,0.63)
    {굵은 선·회색 상자 = 최장 6층 경로 · 1층 값과 최초 입력은 도착 게이트 안에 적었다};
  \node[diagram footer] at (6.50,0.34)
    {\(p_3p_2\)는 항마다 다시 계산했다: AND 2개 증가 · 배선 단순화};
\end{tikzpicture}
\end{bookfigure}

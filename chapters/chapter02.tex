...
첫째 조건의 ``하나 이상''을 \keyterm{OR}(inclusive OR, 논리합)라고 한다. OR는 $A$와 $B$ 가운데 적어도 하나가 $1$이면 $1$이다. 둘 다 $1$이어도 $1$이라는 점이 XOR와 다르다. 둘째 조건은 AND의 결과를 뒤집은 NAND가 바로 알려 준다. 따라서 두 조건을 함께 만족시키면 XOR가 된다.
...
\input{figures/ch02-01-one-function-two-decompositions}
...
\figref{fig:ch02-same-output-different-cost}의 비교는 의도적으로 제한되어 있다. 같은 2입력 NAND를 한 단위로 센 교육용 모형에서는 방법 B가 게이트 두 개 적고 가장 긴 경로도 한 층 짧다. 그러나 실제 칩의 면적과 지연은 배선 길이, 입력 부하, 한 신호가 여러 곳으로 갈라지는 \keyterm{팬아웃}(fan-out), 제조 공정, XOR 전용 셀의 존재에 영향을 받는다. 따라서 이 숫자만으로 방법 B가 모든 실제 칩에서 언제나 더 작고 빠르다고 결론 내릴 수는 없다.

\input{figures/ch02-02-same-output-different-cost}
...
\midrule
설계 대가 & 중간값을 다시 쓰는 분해는 교육 모형의 게이트 수와 경로를 줄이지만 배선·부하·팬아웃을 포함한 실제 비용은 별도 검증이 필요함 \\
\midrule

...
두 구현을 같은 기준으로 표에 놓으면 차이가 더 분명해진다.

\begin{center}
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{0.25\textwidth} >{\centering\arraybackslash}p{0.23\textwidth} >{\centering\arraybackslash}p{0.23\textwidth}}
\toprule
\sffamily\bfseries 구현 & \sffamily\bfseries NAND 수 & \sffamily\bfseries 가장 긴 경로 \\
\midrule
방법 A · 문장을 따라간다 & 6개 & 4층 \\
방법 B · 중간값을 다시 쓴다 & 4개 & 3층 \\
\bottomrule
\end{tabular}
\end{center}

이 비교는 의도적으로 제한되어 있다. 같은 2입력 NAND를 한 단위로 센 모형에서는 방법 B가 게이트 두 개 적고 가장 긴 경로도 한 층 짧다. 이 모형은 배선 길이, 입력 부하와 한 신호가 여러 곳으로 갈라지는 \keyterm{팬아웃}(fan-out)을 세지 않으므로 결론도 그 범위 안에 한정된다.
